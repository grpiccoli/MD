...
namespace ConsultaMD.Controllers
{
    ...
    public class DrMIPController : Controller
    {
        ...
        public async Task<IActionResult> GetPlaces(int insurance, string pwd)
        {
            ...
            if (run.HasValue)
            {
                ...
                return insurance switch
                {
                    0 =>
                        Ok(),
                    1 =>
                        await Fonasa(rut).ConfigureAwait(false),
                    //Banmédica
                    2 =>
                        await Banmedica(rut, pwd).ConfigureAwait(false),
                    //Colmena
                    3 =>
                        await Colmena(rut, pwd).ConfigureAwait(false),
                    //Consalud
                    4 =>
                        await Consalud(rut, pwd).ConfigureAwait(false),
                    //Cruz Blanca
                    5 =>
                        await CruzBlanca(rut, pwd).ConfigureAwait(false),
                    //Nueva Más Vida
                    6 =>
                        await NuevaMasvida(rut, pwd).ConfigureAwait(false),
                    //Vida Tres
                    7 =>
                        await Vida3(rut, pwd).ConfigureAwait(false),
                    _ =>
                        NotFound()
                };
            }
        ...
        {
            var fonasa = await _fonasa.GetByIdAsync(run, true).ConfigureAwait(false);
        ...
        private async Task<IActionResult> Banmedica(int rut, string pwd)
        {
            ...
            using (HttpClient homeClient = new HttpClient(homeHandler))
            {
                ...
                using HttpClientHandler loginHandler = httpClientHandler;
            ...
            using (HttpClient getclient = new HttpClient(handler))
            {
                ...
                using (await getclient.GetAsync(login).ConfigureAwait(false))
                {
                    ...
                    using var doc = await parser
                        .ParseDocumentAsync(await response.Content
